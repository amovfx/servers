    ...
    const results = observations.map((o) => {
      const relation = graph.relations.find(
        (r) =>
          r.from === o.from &&
          r.to === o.to &&
          r.relationType === o.relationType
      );

      if (!relation) {
        throw new Error(
          `Relation from ${o.from} to ${o.to} with type ${o.relationType} not found`
        );
      }

      // Initialize observations array if it doesn't exist
      if (!relation.observations) {
        relation.observations = [];
      }

      const newObservations = o.contents.filter(
        (content) => !relation.observations.includes(content)
      );
      relation.observations.push(...newObservations);
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "Add new observations to existing relations in the knowledge graph",
        inputSchema: {
          type: "object",
          properties: {
            observations: {
              ...
              items: {
                ...
                required: ["from", "to", "relationType", "contents"],
              },
            },
          },
          required: ["observations"],
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "add_relation_observations":
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify(
              await knowledgeGraphManager.addRelationObservations(
                args.observations as {
                  from: string;
                  to: string;
                  relationType: string;
